...
@pytest.mark.asyncio
async def test_get_redis_creates_client_once(monkeypatch):
    # make sure cached redis is cleared before test
    ...
    # mock redis.from_url
    mock_from_url = AsyncMock(return_value=fake_client)
    monkeypatch.setattr(redis_module.redis, "from_url", mock_from_url)
    ...
    assert r1 is fake_client
    assert r2 is fake_client
...
async def test_add_to_blacklist(monkeypatch):
    ...
    await redis_module.add_to_blacklist("token123", 3600)

    fake_redis.set.assert_awaited_once_with("blacklist:token123", "1", ex=3600)
...
async def test_is_blacklisted_true(monkeypatch):
    ...
    result = await redis_module.is_blacklisted("token123")

    assert result is True
    fake_redis.exists.assert_awaited_once_with("blacklist:token123")